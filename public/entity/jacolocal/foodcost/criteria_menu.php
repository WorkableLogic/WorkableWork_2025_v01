...
<form method="post" action="<?php echo url_for('/entity/jacolocal/foodcost/handle_criteria.php'); ?>">
    <input type="hidden" name="criteria_type" value="menu">
    
    <label for="fc_menu_cat_name">Category:</label>
    <select id="fc_menu_cat_name" name="fc_menu_cat_name">
        <option value="All"<?php echo ($criteria['fc_menu_cat_name'] ?? 'All') == 'All' ? ' selected' : ''; ?>>All</option>
        <?php foreach($cats as $cat): ?> 
            <option value="<?= htmlspecialchars($cat->fc_menu_cat_name) ?>" <?= (($criteria['fc_menu_cat_name'] ?? '') == $cat->fc_menu_cat_name) ? ' selected' : ''; ?>>
                <?= htmlspecialchars($cat->fc_menu_cat_name) ?>
            </option>
        <?php endforeach; ?>
    </select>
    
    <label for="fc_menu_group_name">Group:</label>
    <select id="fc_menu_group_name" name="fc_menu_group_name">
        <option value="All"<?php echo ($criteria['fc_menu_group_name'] ?? 'All') == 'All' ? ' selected' : ''; ?>>All</option>
        <?php foreach($groups as $group): ?> 
            <option value="<?= htmlspecialchars($group->fc_menu_group_name) ?>" <?= (($criteria['fc_menu_group_name'] ?? '') == $group->fc_menu_group_name) ? ' selected' : ''; ?>>
